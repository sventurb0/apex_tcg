...
import { canEvolveInto, legalRareCandyBasics } from "./evolution";
import { hasCardTrait, pokemonHasTrait } from "./traits";
import { describeCardInstance, describeChoiceOption, describePokemonTarget } from "../presentation/entity-labels";
...
function setupActions(state: GameState, playerId: PlayerId): GameAction[] {
  ...
  else { if (player.bench.length < benchCapacity(state, playerId)) for (const instance of basicInHand(state, playerId)) actions.push({ id: actionId("setup-bench", instance.instanceId), type: "bench-basic", playerId, cardInstanceId: instance.instanceId, description: `Place ${cardFor(state, instance).name} on the setup Bench` }); actions.push({ id: "finish-setup", type: "finish-setup", playerId, description: "Finish setup" }); }
  return actions;
}

function effectChoiceActions(state: GameState, pending: EffectChoice): GameAction[] {
  if (pending.selectionKind === "mode") return pending.eligibleIds.map((mode) => ({ id: actionId("mode", pending.choiceId, mode), type: "select-effect-mode" as const, playerId: pending.playerId, mode, description: pending.optionLabels?.find((option) => option.id === mode)?.label ?? `Choose ${mode === "both" ? "both modes" : mode}` }));
  ...
  for (const selectionId of pending.eligibleIds) {
    if (selected.has(selectionId)) actions.push({ id: actionId("deselect", pending.choiceId, selectionId), type: "deselect-card", playerId: pending.playerId, selectionId, description: "Deselect choice" });
      ...
      if (pending.continuation.programId === "trainer:brocks-scouting") {
        const player = state.players[pending.playerId];
        const first = player.deck.find((card) => card.instanceId === pending.selectedIds[0]);
        const candidate = player.deck.find((card) => card.instanceId === selectionId);
        const firstDefinition = first ? cardFor(state, first) : undefined;
        const candidateDefinition = candidate ? cardFor(state, candidate) : undefined;
        if (firstDefinition?.category === "pokemon" && candidateDefinition?.category === "pokemon" && (firstDefinition.stage !== "basic" || candidateDefinition.stage !== "basic")) continue;
      }
      ...
      if (pending.selectionKind === "pokemon") actions.push({ id: actionId("select", pending.choiceId, selectionId), type: "select-pokemon", playerId: pending.playerId, selectionId, description: `Select ${describePokemonTarget(state, pending.playerId, selectionId)}` });
      else actions.push({ id: actionId("select", pending.choiceId, selectionId), type: "select-card", playerId: pending.playerId, selectionId, description: `Select ${pending.optionLabels?.find((option) => option.id === selectionId)?.label ?? describeCardInstance(state, pending.playerId, selectionId)}` });
    }
  }
...
function allocationChoiceActions(state: GameState, pending: AllocationChoice): GameAction[] {
  ...
  for (const targetId of pending.eligibleIds) {
    ...
    const label = describeChoiceOption(state, pending.playerId, targetId, "pokemon");
    if (current < max && pending.remainingUnits > 0) actions.push({ id: actionId("increase-allocation", pending.choiceId, targetId), type: "increase-allocation", playerId: pending.playerId, targetId, amount: 1, description: `Assign 1 ${pending.unitLabel} to ${label}` });
    if (current > 0) actions.push({ id: actionId("decrease-allocation", pending.choiceId, targetId), type: "decrease-allocation", playerId: pending.playerId, targetId, amount: 1, description: `Remove 1 ${pending.unitLabel} from ${label}` });
    for (let amount = 0; amount <= Math.min(max, pending.totalUnits); amount += 1) if (amount !== current && pending.totalUnits - (pending.totalUnits - pending.remainingUnits - current) - amount >= 0) actions.push({ id: actionId("set-allocation", pending.choiceId, targetId, String(amount)), type: "set-allocation", playerId: pending.playerId, targetId, amount, description: `Set ${label} to ${amount} ${pending.unitLabel}` });
  }
...
function retreatCost(state: GameState, pokemon: PokemonInPlay): number { const card = topCard(state, pokemon); const skyliner = pokemonTargets(state.players[state.activePlayerId]).some((candidate) => topCard(state, candidate).abilities.some((ability) => ability.effectProgramId === "passive:skyliner")); if (card.abilities.some((ability) => ability.effectProgramId === "template:ability:free-retreat") || skyliner && card.stage === "basic") return 0; const tool = pokemon.tool ? cardFor(state, pokemon.tool) : null; const reduction = tool?.category === "trainer" && tool.effectProgramId === "tool:air-balloon" ? 2 : tool?.category === "trainer" && tool.effectProgramId === "light-boots" ? 1 : 0; return Math.max(0, card.retreatCost - reduction); }
...
export function getLegalActions(state: GameState, playerId: PlayerId): GameAction[] {
  ...
  if (pending) {
    ...
    if (pending.type === "allocation-choice") return allocationChoiceActions(state, pending);
  ...
  for (const instance of player.hand) {
    ...
    if (card.category === "pokemon" && card.stage === "basic" && player.bench.length < benchCapacity(state, playerId)) actions.push({ id: actionId("bench", instance.instanceId), type: "bench-basic", playerId, cardInstanceId: instance.instanceId, description: `Bench ${card.name}` });
    ...
    if (card.category === "pokemon" && card.stage !== "basic") for (const target of pokemonTargets(player)) if (canEvolveInto(state, playerId, target, instance)) actions.push({ id: actionId("evolve", instance.instanceId, playId(target)), type: "evolve", playerId, cardInstanceId: instance.instanceId, targetId: playId(target), description: `Evolve ${topCard(state, target).name} into ${card.name}` });
    if (card.category === "trainer") {
      ...
      if (card.effectProgramId === "trainer:briar" && state.players[playerId === "player-one" ? "player-two" : "player-one"].prizes.length !== 2) continue;
      ...
      if (card.effectProgramId === "trainer:prime-catcher" && !state.players[playerId === "player-one" ? "player-two" : "player-one"].bench.length) continue;
      if (card.effectProgramId === "trainer:lisias-appeal" && !state.players[playerId === "player-one" ? "player-two" : "player-one"].bench.some((pokemon) => topCard(state, pokemon).stage === "basic")) continue;
      ...
      if (card.effectProgramId === "trainer:unfair-stamp" && !state.events.some((event) => event.type === "pokemon-knocked-out" && event.turn === state.turn - 1 && event.targetPlayerId === playerId)) continue;
      if (card.effectProgramId === "trainer:special-red-card" && state.players[playerId === "player-one" ? "player-two" : "player-one"].prizes.length > 3) continue;
  ...
  for (const source of pokemonTargets(player)) for (const ability of topCard(state, source).abilities) {
    const stadiumDefinition = state.stadium ? cardFor(state, state.stadium) : undefined;
    const watchtowerBlocks = stadiumDefinition?.category === "trainer" && stadiumDefinition.effectProgramId === "stadium:team-rocket-watchtower" && topCard(state, source).pokemonType === "colorless";
    if (watchtowerBlocks) continue;
    ...
    if (state.temporaryEffects.some((effect) => effect.kind === "ability-lock" && effect.playerId === playerId && (!effect.pokemonId || effect.pokemonId === playId(source)) && effect.appliesOnPlayerTurn === player.turnsTaken)) continue;
    ...
    else if ((ability.effectProgramId === "ability:run-errand" || ability.effectProgramId === "ability:allure") && source === player.active && player.deck.length > 0) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: `Use ${ability.name}` });
    else if (ability.effectProgramId === "ability:run-away-draw" && player.deck.length > 0) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: "Use Run Away Draw" });
    else if (ability.effectProgramId === "ability:invite-evil" && player.deck.some((card) => { const definition = cardFor(state, card); return definition.category === "pokemon" && definition.pokemonType === "darkness"; })) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: "Use Invite Evil" });
    else if (ability.effectProgramId === "ability:come-and-get-you" && player.discard.some((card) => cardFor(state, card).id === "sv8pt5-35") && player.bench.length < benchCapacity(state, playerId)) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: "Use Come and Get You" });
    else if ((ability.effectProgramId === "ability:cursed-blast-5" || ability.effectProgramId === "ability:cursed-blast-13") && pokemonTargets(state.players[playerId === "player-one" ? "player-two" : "player-one"]).length) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: `Use ${ability.name}` });
    else if (ability.effectProgramId === "ability:seething-spirit" && player.discard.some((card) => { const definition = cardFor(state, card); return definition.category === "energy" && definition.basic; })) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: "Use Seething Spirit" });
    else if (ability.effectProgramId === "ability:jewel-seeker" && pokemonTargets(player).some((pokemon) => topCard(state, pokemon).traits?.includes("tera")) && player.deck.some((card) => cardFor(state, card).category === "trainer")) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: "Use Jewel Seeker" });
    ...
    else if (ability.effectProgramId === "ability:teleporter" && source === player.active && player.bench.length > 0) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: "Use Teleporter" });
    else if (ability.effectProgramId === "ability:lunar-cycle" && player.hand.some((instance) => { const card = cardFor(state, instance); return card.category === "energy" && card.basic && card.energyType === "fighting"; }) && pokemonTargets(player).some((pokemon) => topCard(state, pokemon).name === "Solrock")) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: "Use Lunar Cycle" });
    else if (ability.effectProgramId === "ability:boom-boom-groove" && player.deck.length > 0 && player.active && topCard(state, player.active).abilities.some((candidate) => candidate.effectProgramId === "passive:festival-lead")) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: "Use Boom Boom Groove" });
    else if (ability.effectProgramId === "ability:mortal-shuriken" && source === player.active && player.hand.some((instance) => { const card = cardFor(state, instance); return card.category === "energy" && card.basic && card.energyType === "water"; }) && pokemonTargets(state.players[playerId === "player-one" ? "player-two" : "player-one"]).length) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: "Use Mortal Shuriken" });
    else if (ability.effectProgramId === "ability:metal-maker" && player.deck.length > 0) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: "Use Metal Maker" });
    else if (ability.effectProgramId === "ability:metallic-signal" && player.deck.some((instance) => { const card = cardFor(state, instance); return card.category === "pokemon" && card.pokemonType === "metal" && card.stage !== "basic"; })) actions.push({ id: actionId("ability", ability.id, playId(source)), type: "use-ability", playerId, sourcePokemonId: playId(source), abilityId: ability.id, description: "Use Metallic Signal" });
  ...
  if (stadium?.category === "trainer" && stadium.effectProgramId === "stadium:academy-at-night" && !player.stadiumAbilityUsedThisTurn) for (const card of player.hand) actions.push({ id: actionId("stadium", stadium.id, card.instanceId), type: "use-stadium", playerId, cardInstanceId: state.stadium!.instanceId, targetId: card.instanceId, description: `Use Academy at Night: put ${cardFor(state, card).name} on top of your deck` });
  ...
  if (player.active) {
    ...
    const festivalLead = Boolean(state.stadium && cardFor(state, state.stadium).name === "Festival Grounds" && activeCard.abilities.some((ability) => ability.effectProgramId === "passive:festival-lead"));
    const attacksUsed = player.attacksUsedThisTurn ?? 0;
    const canAttack = attacksUsed === 0 || festivalLead && attacksUsed === 1;
    const attackPrevented = state.temporaryEffects.some((effect) => effect.kind === "attack-prevention" && effect.playerId === playerId && effect.pokemonId === playId(player.active!) && effect.appliesOnPlayerTurn === player.turnsTaken);
    for (const attack of activeCard.attacks) if (canAttack && !attackPrevented && state.players[playerId === "player-one" ? "player-two" : "player-one"].active && !(playerId === state.startingPlayer && player.turnsTaken === 1) && !blocked && !(activeCard.abilities.some((ability) => ability.effectProgramId === "passive:power-saver") && [player.active, ...player.bench].filter((pokemon): pokemon is PokemonInPlay => Boolean(pokemon)).filter((pokemon) => topCard(state, pokemon).traits?.includes("team-rocket")).length < 4) && !hasAttackLock(state, playerId, playId(player.active), attack.id) && (!attack.condition || canUseAttackCondition(state, playerId, player.active, attack.condition)) && canPayAttackCost(state, player.active, attack)) {
