import { continueAllocationEffectProgram, continueEffectProgram, recordEffectExecution, startEffectProgram } from "../effects/program-runner";
import type { GameAction, PlayerId } from "../model/actions";
import type { CardInstance, PokemonInPlay } from "../model/cards";
import type { AllocationChoice, EffectChoice, GameState, KnockOutCause, PendingKnockOutCause, PlayerState, ResolutionResume } from "../model/game-state";
...
import { applySpecialCondition, clearSpecialConditions, processPokemonCheckup } from "./pokemon-checkup";
import { attackDamageBonus, effectiveMaxHp, modifiedPrizeValue } from "./modifiers";
...
import { canAttachEnergyToPokemon, enforceAttachmentValidity } from "./attachment-validity";
import { hasCardTrait } from "./traits";
import { canEvolveInto } from "./evolution";
import { MissingEffectProgramError } from "../effects/errors";
...
function startTurn(state: GameState, playerId: PlayerId): void {
  ...
  player.energyAttachedThisTurn = false; player.supporterPlayedThisTurn = false; player.stadiumPlayedThisTurn = false; player.stadiumAbilityUsedThisTurn = false; player.retreatedThisTurn = false; player.attacksUsedThisTurn = 0; player.turnsTaken += 1;
...
function finishEffect(state: GameState, choice: EffectChoice | AllocationChoice): void { state.pendingChoice = null; if (choice.continuation.after === "finish-attack") finishAttack(state, choice.continuation.actingPlayerId, choice.continuation.sourceCardId); else if (state.pendingKnockOutCause) resolveKnockOuts(state, { kind: "resume-main", playerId: choice.continuation.actingPlayerId }, state.pendingKnockOutCause); else { state.phase = "main"; } }
...
function applyTrainer(state: GameState, action: Extract<GameAction, { type: "play-trainer" }>): void {
  ...
  if (card.subtype === "tool" && action.targetId) { const target = findPokemon(player, action.targetId); if (!target) throw new GameRuleError("Tool target is unavailable.", action); target.tool = instance; recordEffectExecution(state, card.effectProgramId, card.id, "completed", ["tool-attached"]); return; }
  if (card.subtype === "tool") { player.discard.push(instance); recordEffectExecution(state, card.effectProgramId, card.id, "declined", ["tool-no-target"]); return; }
  if (card.effectProgramId === "trainer:transformation-tome") {
    const secondIndex = player.hand.findIndex((candidate) => candidate.cardId === instance.cardId);
    if (secondIndex < 0) { player.discard.push(instance); return; }
    const second = player.hand.splice(secondIndex, 1)[0]!; player.discard.push(instance, second);
    const discardBasics = player.discard.filter((candidate) => { const def = cardFor(state, candidate); return def.category === "pokemon" && def.stage === "basic"; }).map((candidate) => candidate.instanceId);
    const playBasics = pokemonTargets(player).filter((pokemon) => topCard(state, pokemon).stage === "basic").map(playId);
    if (!discardBasics.length || !playBasics.length) return;
    state.phase = "choice"; state.pendingChoice = { type: "effect-choice", choiceId: `transformation-tome:${state.actionHistory.length}`, playerId: action.playerId, selectionKind: "card", min: 1, max: 1, eligibleIds: discardBasics, selectedIds: [], optional: false, instruction: "Choose a Basic Pokémon from your discard pile.", sourceCardId: card.id, sourceEffectId: card.effectProgramId, continuation: { programId: card.effectProgramId, step: 1, actingPlayerId: action.playerId, sourceCardId: card.id, variables: { playTargets: playBasics }, after: "resume-main" } };
    return;
  }
  player.discard.push(instance);
  if (card.effectProgramId === "trainer:switch" && action.targetId) { const active = player.active; const index = player.bench.findIndex((pokemon) => playId(pokemon) === action.targetId); if (!active || index < 0) { player.discard.push(instance); return; } clearSpecialConditions(active); player.active = player.bench.splice(index, 1, active)[0]!; return; }
  ...
  const completed = startEffectProgram(state, { programId: card.effectProgramId, actingPlayerId: action.playerId, sourceCardId: card.id, sourceInstanceId: instance.instanceId, after: "resume-main" });
  recordEffectExecution(state, card.effectProgramId, card.id, completed ? "completed" : "started", ["card-played"]); if (completed) state.phase = "main";
}

function applyAbility(state: GameState, action: Extract<GameAction, { type: "use-ability" }>): void {
  const player = state.players[action.playerId]; const source = findPokemon(player, action.sourcePokemonId); if (!source) throw new GameRuleError("Ability user is unavailable.", action); const sourceId = playId(source);
  ...
  else { startEffectProgram(state, { programId: ability.effectProgramId, actingPlayerId: action.playerId, sourceCardId: card.id, sourcePokemonId: sourceId, after: "resume-main", variables: { ...(action.targetId ? { target: [action.targetId] } : {}), ...(action.cardInstanceId ? { card: [action.cardInstanceId] } : {}) } }); }
  ...
  emitEvent(state, "ability-used", action.playerId, { sourceCardId: card.id, targetId: sourceId, detail: ability.name });
}
...
function applyStadiumAbility(state: GameState, action: Extract<GameAction, { type: "use-stadium" }>): void {
  ...
  if (stadium?.category === "trainer" && stadium.effectProgramId === "stadium:academy-at-night") { const player = state.players[action.playerId]; if (player.stadiumAbilityUsedThisTurn) throw new GameRuleError("Academy at Night was already used this turn.", action); const card = removeFromHand(player, action.targetId); player.deck.unshift(card); player.stadiumAbilityUsedThisTurn = true; emitEvent(state, "stadium-ability-used", action.playerId, { sourceCardId: stadium.id, cardInstanceIds: [card.instanceId], detail: "Academy at Night put a card on top of the deck" }); return; }
...
function applyAttack(state: GameState, action: Extract<GameAction, { type: "attack" }>): void {
  ...
  const baseDamage = resolveBaseDamage(attack.damage, attacker.active, state, action.playerId); const bonus = attackDamageBonus(state, attacker.active, targetIsActive); const benchProtected = !targetIsActive && (defendingCard.id === "sv6-130" || defender.bench.some((pokemon) => pokemon === targeted && topCard(state, pokemon).abilities.some((ability) => ["passive:spherical-shield", "passive:flower-curtain"].includes(ability.effectProgramId)))); const rockInnProtected = targetIsActive && defendingCard.abilities.some((ability) => ability.effectProgramId === "passive:mysterious-rock-inn") && attackingCard.isPokemonEx; const incomingAttackPrevented = state.temporaryEffects.some((effect) => effect.kind === "incoming-attack-prevention" && effect.playerId === action.playerId && effect.pokemonId === playId(targeted) && effect.appliesOnPlayerTurn === attacker.turnsTaken); const fairyZone = pokemonTargets(attacker).some((pokemon) => topCard(state, pokemon).abilities.some((ability) => ability.effectProgramId === "passive:fairy-zone")); const effectiveDefendingCard = fairyZone && defendingCard.pokemonType === "dragon" ? { ...defendingCard, weakness: { type: "psychic" as const, multiplier: 2 } } : defendingCard; const ignoresWeaknessResistance = attack.damage.kind === "formula" && attack.damage.resolverId === "cosmic-beam-damage"; const damage = benchProtected || rockInnProtected || incomingAttackPrevented ? 0 : targetIsActive ? ignoresWeaknessResistance ? baseDamage : calculateDamage(attackingCard, effectiveDefendingCard, baseDamage + bonus.amount) : baseDamage;
  ...
  const resoluteHeart = targetIsActive && targeted.damage === 0 && defendingCard.abilities.some((ability) => ability.effectProgramId === "passive:resolute-heart") && damage >= effectiveMaxHp(state, targeted); targeted.damage += resoluteHeart ? Math.max(0, effectiveMaxHp(state, targeted) - 10) : damage; if (damage) emitEvent(state, "damage-dealt", action.playerId, { sourceCardId: attackingCard.id, targetId: playId(targeted), amount: damage }); if (targetIsActive && isKnockedOut(state, targeted) && !resoluteHeart) state.pendingKnockOutCause = { cause: "attack-damage", sourcePlayerId: action.playerId, sourceCardId: attackingCard.id }; if (bonus.amount) emitEvent(state, "damage-modifier-applied", action.playerId, { sourceCardId: bonus.sourceCardId, targetId: playId(targeted), amount: bonus.amount, detail: "Damage modifier" });
  const spiky = targetIsActive && damage > 0 && targeted.attachedEnergy.some((energy) => cardFor(state, energy).category === "energy" && cardFor(state, energy).effectProgramId === "energy:spiky"); if (spiky) { attacker.active!.damage += 20; emitEvent(state, "damage-dealt", defenderId, { sourceCardId: "sv9-159", targetId: playId(attacker.active!), targetPlayerId: action.playerId, amount: 20, detail: "Spiky Energy damage counters" }); if (isKnockedOut(state, attacker.active!)) state.pendingKnockOutCause = { cause: "effect-damage-counters", sourcePlayerId: defenderId, sourceCardId: "sv9-159" }; }
  if (targetIsActive && damage > 0 && defender.active && defendingCard.abilities.some((ability) => ability.effectProgramId === "passive:incandescent-body")) applySpecialCondition(attacker.active, "burned");
  if (targetIsActive && damage > 0 && defender.active?.tool && cardFor(state, defender.active.tool).category === "trainer" && cardFor(state, defender.active.tool).effectProgramId === "tool:lucky-helmet") defender.hand.push(...defender.deck.splice(0, 2));
  if (attack.effectProgramId) { const energyEffectBlocked = targeted.attachedEnergy.some((energy) => { const definition = cardFor(state, energy); return definition.category === "energy" && (definition.effectProgramId === "energy:mist" || definition.effectProgramId === "energy:rocky-fighting" && defendingCard.pokemonType === "fighting"); }); const effectsBlocked = incomingAttackPrevented || energyEffectBlocked || defendingCard.stage === "basic" && hasCardTrait(defendingCard, "team-rocket") && pokemonTargets(defender).some((pokemon) => topCard(state, pokemon).abilities.some((ability) => ability.effectProgramId === "passive:repelling-veil")); if (!effectsBlocked) { const completed = startEffectProgram(state, { programId: attack.effectProgramId, actingPlayerId: action.playerId, sourceCardId: attackingCard.id, sourcePokemonId: playId(attacker.active), attackId: attack.id, after: "finish-attack", variables: action.targetId ? { target: [action.targetId] } : undefined }); if (!completed) return; } }
...
function confirmChoice(state: GameState, selectedOverride?: string[]): void { const pending = state.pendingChoice; if (pending?.type !== "effect-choice") throw new GameRuleError("No effect choice is pending."); const selected = selectedOverride ?? pending.selectedIds; if (selected.length < pending.min || selected.length > pending.max) throw new GameRuleError(`Select between ${pending.min} and ${pending.max}.`); state.pendingChoice = null; const completed = continueEffectProgram(state, pending, selected); if (completed && state.pendingChoice === null) finishEffect(state, pending); }
function updateAllocation(state: GameState, action: Extract<GameAction, { type: "increase-allocation" | "decrease-allocation" | "set-allocation" }>): void {
  const pending = state.pendingChoice;
  if (pending?.type !== "allocation-choice" || !pending.eligibleIds.includes(action.targetId)) throw new GameRuleError("Allocation target is not eligible.", action);
  const current = pending.allocations[action.targetId] ?? 0;
  const amount = action.type === "increase-allocation" ? current + (action.amount ?? 1) : action.type === "decrease-allocation" ? current - (action.amount ?? 1) : action.amount;
  const maximum = pending.maximumPerTarget ?? pending.totalUnits;
  if (!Number.isInteger(amount) || amount < (pending.minimumPerTarget ?? 0) || amount > maximum) throw new GameRuleError("Allocation amount is invalid.", action);
  const allocatedElsewhere = pending.totalUnits - pending.remainingUnits - current;
  if (allocatedElsewhere + amount > pending.totalUnits) throw new GameRuleError("Allocation exceeds the available pool.", action);
  const allocations = { ...pending.allocations };
  if (amount === 0) delete allocations[action.targetId]; else allocations[action.targetId] = amount;
  state.pendingChoice = { ...pending, allocations, remainingUnits: pending.totalUnits - allocatedElsewhere - amount };
}
function clearAllocation(state: GameState): void { const pending = state.pendingChoice; if (pending?.type !== "allocation-choice") throw new GameRuleError("No allocation is pending."); state.pendingChoice = { ...pending, allocations: {}, remainingUnits: pending.totalUnits }; }
function confirmAllocation(state: GameState): void { const pending = state.pendingChoice; if (pending?.type !== "allocation-choice") throw new GameRuleError("No allocation is pending."); if (pending.remainingUnits !== 0) throw new GameRuleError(`Allocate exactly ${pending.totalUnits} ${pending.unitLabel}s.`); state.pendingChoice = null; const completed = continueAllocationEffectProgram(state, pending); if (completed && state.pendingChoice === null) finishEffect(state, pending); }

function mutate(state: GameState, action: GameAction): void {
  const player = state.players[action.playerId];
  switch (action.type) {
    case "select-active": applySelectActive(state, action); break;
    case "bench-basic": { if (player.bench.length >= benchCapacity(state, action.playerId)) throw new GameRuleError("The Bench is full.", action); const instance = removeFromHand(player, action.cardInstanceId); const pokemon = makePokemon(instance, state.turn); player.bench.push(pokemon); emitEvent(state, "pokemon-benched", action.playerId, { sourceCardId: instance.cardId, sourceInstanceId: instance.instanceId, targetId: playId(pokemon) }); if (instance.cardId === "me3-62" && !player.abilityUsageByName["Last-Ditch Catch"]) { const eligibleIds = player.deck.filter((candidate) => { const definition = cardFor(state, candidate); return definition.category === "trainer" && definition.subtype === "supporter"; }).map((candidate) => candidate.instanceId); player.abilityUsageByName["Last-Ditch Catch"] = state.turn; if (eligibleIds.length) { state.phase = "choice"; state.pendingChoice = { type: "effect-choice", choiceId: `last-ditch-catch:${state.actionHistory.length}`, playerId: action.playerId, selectionKind: "card", min: 0, max: 1, eligibleIds, selectedIds: [], optional: true, instruction: "Choose up to 1 Supporter for Last-Ditch Catch.", sourceCardId: instance.cardId, sourceEffectId: "ability:last-ditch-catch", continuation: { programId: "ability:last-ditch-catch", step: 1, actingPlayerId: action.playerId, sourceCardId: instance.cardId, sourcePokemonId: playId(pokemon), variables: {}, after: "resume-main" } }; } } break; }
    ...
    case "attach-energy": { const target = findPokemon(player, action.targetId); if (!target) throw new GameRuleError("Energy target is unavailable.", action); const instance = removeFromHand(player, action.cardInstanceId); const definition = cardFor(state, instance); if (definition.category !== "energy") throw new GameRuleError("Only Energy cards can be attached.", action); if (!canAttachEnergyToPokemon(state, instance, target)) { player.hand.push(instance); throw new GameRuleError("That Energy cannot be attached to the selected Pokémon.", action); } target.attachedEnergy.push(instance); player.energyAttachedThisTurn = true; emitEvent(state, "energy-attached-manually", action.playerId, { sourceCardId: instance.cardId, targetId: playId(target), cardInstanceIds: [instance.instanceId], detail: instance.cardId === "sv10-182" ? "Team Rocket's Energy" : undefined }); if (definition.effectProgramId === "energy:enriching") player.hand.push(...player.deck.splice(0, 4)); if (definition.effectProgramId === "energy:telepathic-psychic" && topCard(state, target).pokemonType === "psychic") startEffectProgram(state, { programId: "energy:telepathic-psychic", actingPlayerId: action.playerId, sourceCardId: instance.cardId, sourceInstanceId: instance.instanceId, sourcePokemonId: playId(target), after: "resume-main" }); break; }
    case "evolve": { const target = findPokemon(player, action.targetId); if (!target) throw new GameRuleError("Evolution target is unavailable.", action); const instance = player.hand.find((candidate) => candidate.instanceId === action.cardInstanceId); if (!instance || !canEvolveInto(state, action.playerId, target, instance)) throw new GameRuleError("Evolution is not legal this turn.", action); removeFromHand(player, action.cardInstanceId); target.stack.push(instance); target.evolvedThisTurn = true; clearSpecialConditions(target); emitEvent(state, "pokemon-evolved", action.playerId, { sourceCardId: instance.cardId, targetId: playId(target) }); const stadium = state.stadium ? cardFor(state, state.stadium) : undefined; if (stadium?.category === "trainer" && stadium.effectProgramId === "stadium:gravity-mountain") resolveKnockOuts(state, { kind: "resume-main", playerId: action.playerId }, { cause: "stadium-effect", sourcePlayerId: action.playerId, sourceCardId: stadium.id }); break; }
    ...
    case "retreat": { if (!player.active) throw new GameRuleError("There is no Active Pokémon to retreat.", action); const targetIndex = player.bench.findIndex((pokemon) => playId(pokemon) === action.targetId); if (targetIndex < 0) throw new GameRuleError("Retreat target is unavailable.", action); const skyliner = pokemonTargets(player).some((pokemon) => topCard(state, pokemon).abilities.some((ability) => ability.effectProgramId === "passive:skyliner")); const activeTool = player.active.tool ? cardFor(state, player.active.tool) : undefined; const toolReduction = activeTool?.category === "trainer" && activeTool.effectProgramId === "tool:air-balloon" ? 2 : activeTool?.category === "trainer" && activeTool.effectProgramId === "light-boots" ? 1 : 0; const cost = skyliner && topCard(state, player.active).stage === "basic" ? 0 : Math.max(0, topCard(state, player.active).retreatCost - toolReduction); if (cost > player.active.attachedEnergy.length) throw new GameRuleError("There is not enough Energy to pay the Retreat Cost.", action); if (!cost || player.active.attachedEnergy.length === cost) { if (cost) player.discard.push(...player.active.attachedEnergy.splice(0, cost)); clearSpecialConditions(player.active); const target = player.bench.splice(targetIndex, 1, player.active)[0]!; player.active = target; player.retreatedThisTurn = true; } else { state.phase = "choice"; state.pendingChoice = { type: "effect-choice", choiceId: `retreat-energy:${state.actionHistory.length}`, playerId: action.playerId, selectionKind: "card", min: cost, max: cost, eligibleIds: player.active.attachedEnergy.map((energy) => energy.instanceId), selectedIds: [], optional: false, instruction: `Choose ${cost} Energy to discard for Retreat.`, sourceCardId: topCard(state, player.active).id, sourceEffectId: "retreat-energy", continuation: { programId: "retreat-energy", step: 1, actingPlayerId: action.playerId, sourceCardId: topCard(state, player.active).id, sourcePokemonId: playId(player.active), variables: { target: [action.targetId] }, after: "resume-main" } }; } break; }
    ...
    case "increase-allocation": case "decrease-allocation": case "set-allocation": updateAllocation(state, action); break;
    case "clear-allocation": clearAllocation(state); break;
    case "confirm-allocation": confirmAllocation(state); break;
...
export function applyAction(state: GameState, submittedAction: GameAction): GameState {
  ...
  try { mutate(next, canonical); } catch (error) { if (error instanceof MissingEffectProgramError) { error.turn = next.turn; error.actingPlayerId = canonical.playerId; error.actionHistory = next.actionHistory.slice(-12); } throw error; }
  enforceAttachmentValidity(next); next.actionHistory.push(canonical); if (next.detailedLogs) next.actionLog.push({ index: next.actionLog.length, turn: state.turn, playerId: canonical.playerId, actionId: canonical.id, description: canonical.description }); return next;
}
