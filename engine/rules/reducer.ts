...
function applyTrainer(state: GameState, action: Extract<GameAction, { type: "play-trainer" }>): void {
  ...
  try { completed = startEffectProgram(state, { programId: card.effectProgramId, actingPlayerId: action.playerId, sourceCardId: card.id, sourceInstanceId: instance.instanceId, after: "resume-main" }); }
  catch (error) { if (!(error instanceof Error && error.name === "MissingEffectProgramError")) throw error; recordEffectExecution(state, card.effectProgramId, card.id, "declined", ["missing-program-guard"]); state.phase = "main"; return; }
...
function applyStadiumAbility(state: GameState, action: Extract<GameAction, { type: "use-stadium" }>): void {
  ...
  if (stadium?.category === "trainer" && stadium.effectProgramId === "stadium:academy-at-night") { const player = state.players[action.playerId]; if (player.stadiumAbilityUsedThisTurn) throw new GameRuleError("Academy at Night was already used this turn.", action); if ((player.academyUsesThisGame ?? 0) >= 8) throw new GameRuleError("Academy at Night has reached its repetition guard for this game.", action); const card = removeFromHand(player, action.targetId); player.deck.unshift(card); player.stadiumAbilityUsedThisTurn = true; player.academyUsesThisGame = (player.academyUsesThisGame ?? 0) + 1; emitEvent(state, "stadium-ability-used", action.playerId, { sourceCardId: stadium.id, cardInstanceIds: [card.instanceId], detail: "Academy at Night put a card on top of the deck" }); return; }
...
function applyAttack(state: GameState, action: Extract<GameAction, { type: "attack" }>): void {
  ...
  if (attack.effectProgramId) { const effectsBlocked = defendingCard.stage === "basic" && hasCardTrait(defendingCard, "team-rocket") && pokemonTargets(defender).some((pokemon) => topCard(state, pokemon).abilities.some((ability) => ability.effectProgramId === "passive:repelling-veil")); if (!effectsBlocked) { let completed = true; try { completed = startEffectProgram(state, { programId: attack.effectProgramId, actingPlayerId: action.playerId, sourceCardId: attackingCard.id, sourcePokemonId: playId(attacker.active), attackId: attack.id, after: "finish-attack", variables: action.targetId ? { target: [action.targetId] } : undefined }); } catch (error) { if (!(error instanceof Error && error.name === "MissingEffectProgramError")) throw error; recordEffectExecution(state, attack.effectProgramId, attackingCard.id, "declined", ["missing-program-guard"]); } if (!completed) return; } }
...
function confirmAllocation(state: GameState): void { const pending = state.pendingChoice; if (pending?.type !== "allocation-choice") throw new GameRuleError("No allocation is pending."); if (pending.remainingUnits !== 0) throw new GameRuleError(`Allocate exactly ${pending.totalUnits} ${pending.unitLabel}s.`); state.pendingChoice = null; const completed = continueAllocationEffectProgram(state, pending); if (completed) finishEffect(state, pending); }
