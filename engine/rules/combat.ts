...
export function canPayAttackCost(state: GameState, pokemon: PokemonInPlay, attack: AttackDefinition): boolean {
  ...
  const anyTypes: CardType[] = ["grass", "fire", "water", "lightning", "psychic", "fighting", "darkness", "metal", "colorless"];
  for (const card of energy) { let options = card.supplyOptions?.length ? card.supplyOptions : [{ [card.energyType]: 1 }]; if (card.effectProgramId === "energy:ignition" && topCard(state, pokemon).stage !== "basic") options = [{ colorless: 3 }]; if (card.effectProgramId === "energy:neo-upper" && topCard(state, pokemon).stage === "stage2") options = anyTypes.flatMap((first) => anyTypes.map((second) => ({ [first]: 1, [second]: ((first === second) ? 2 : 1) }))); totals = totals.flatMap((total) => options.map((option) => { const combined = { ...total }; for (const [type, units] of Object.entries(option) as [CardType, number][]) combined[type] = (combined[type] ?? 0) + units; return combined; })); }
