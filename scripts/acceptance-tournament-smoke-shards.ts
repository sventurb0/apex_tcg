...
for (const deck of targets) {
  ...
  const shard = { sourceDeckId, deckName: deck.snapshot.archetype, expectedGames: 80, games: games.length, unresolved: unresolved.length, invalidNumeric: games.reduce((sum, game) => sum + game.invalidNumericCount, 0), choiceLoops: games.filter((game) => /choice loop/i.test(game.result.reason ?? "")).length, noLegalAction: games.filter((game) => /no legal action/i.test(game.result.reason ?? "")).length, turnLimit: games.filter((game) => /turn limit/i.test(game.result.reason ?? "")).length, seeds: games.map((game) => game.seed), unresolvedDetails: unresolved.map((game) => ({ seed: game.seed, reason: game.result.reason, finalState: game.finalStateSummary, tail: game.actionSequence.slice(-8) })) };
